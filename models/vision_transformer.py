"""This code is based on OODformer git-hub code"""
import torch
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        self.score = attn_weights
        if mask is not None:
            mask = mask[:, None, None, :].float()
            self.score -= 10000.0 * (1.0 - mask)
        self.score = self.dropout(F.softmax(self.score, dim=-1))
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, return_feat=False, not_contrastive_acc=False, eval=False, feat_cls=False):
        ...
        #     print(f'feat.requires_grad: {feat.requires_grad}')  # False
