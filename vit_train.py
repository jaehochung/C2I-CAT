...
from utils import save_vit_model
...
def set_lr_scheduler(args, optimizer, train_loader):
    if args.vit_cos_decay:
        print(f'cosine decay..')
        scheduler = optim.lr_scheduler.LambdaLR(
            optimizer,
            lr_lambda=lambda step: cosine_annealing(
                step,
                args.vit_epoch * len(train_loader),
                1,
                1e-06 / args.vit_lr
            )
        )

    elif args.vit_onecycle and not args.vit_cos_decay:
        print(f'one cycle..')
...
def main(args):
    ...
    # last model save
    save_vit_model(model, tr_acc, te_acc, save_path)
    print('=' * 100)
